        ...
        unchecked
        {
            ...
            switch (obj)
            {
                ...
                default:
                    ...
                    throw new InvalidOperationException(string.Format("Unhandled expression node type '{0}'.", obj.NodeType));
            }
    ...
    private struct ExpressionComparer
    {
        ...
        public bool Compare(Expression? left, Expression? right)
        {
            ...
            return left switch
            {
                ...
                _ => left.NodeType == ExpressionType.Extension
                    ? left.Equals(right)
                        : throw new InvalidOperationException(string.Format("Unhandled expression node type '{0}'.", left.NodeType))
            };
        }
        ...
        private bool CompareLambda(LambdaExpression a, LambdaExpression b)
        {
            ...
            for (var i = 0; i < n; i++)
            {
                ...
                {
                    throw new InvalidOperationException(string.Format("The same parameter instance with name '{0}' was used in multiple lambdas in the query tree. Each lambda must have its own parameter instances.", p1.Name));
                }
            }
...
#pragma warning restore IDE0066 // Convert switch statement to expression
            {
                ...
                default:
                    throw new InvalidOperationException(string.Format("Unhandled member binding type '{0}'.", a.BindingType));
            }
        }
