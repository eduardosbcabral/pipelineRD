...
public abstract class Handler<TContext, TRequest> where TContext : BaseContext
{
    ...
    protected void Abort(string errorMessage, int httpStatusCode)
        => Context.Result = HandlerResultBuilder.CreateDefault().WithError(new(errorMessage)).WithStatusCode((HttpStatusCode)httpStatusCode);

    protected void Abort(string errorMessage, HttpStatusCode httpStatusCode)
        => Context.Result = HandlerResultBuilder.CreateDefault().WithError(new(errorMessage)).WithStatusCode(httpStatusCode);

    protected void Abort(HandlerError error, HttpStatusCode httpStatusCode)
        => Context.Result = HandlerResultBuilder.CreateDefault().WithError(error).WithStatusCode(httpStatusCode);

    protected void Abort(HandlerError error, int httpStatusCode)
        => Context.Result = HandlerResultBuilder.CreateDefault().WithError(error).WithStatusCode((HttpStatusCode)httpStatusCode);

    protected void Abort(IEnumerable<HandlerError> errors, HttpStatusCode httpStatusCode)
        => Context.Result = HandlerResultBuilder.CreateDefault().WithErrors(errors).WithStatusCode(httpStatusCode);

    protected void Abort(IEnumerable<HandlerError> errors, int httpStatusCode)
        => Context.Result = HandlerResultBuilder.CreateDefault().WithErrors(errors).WithStatusCode((HttpStatusCode)httpStatusCode);

    protected void Finish(object result, HttpStatusCode httpStatusCode)
        => Context.Result = HandlerResultBuilder.CreateDefault().WithResult(result).WithStatusCode(httpStatusCode);
