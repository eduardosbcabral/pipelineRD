...
namespace PipelineRD
{
    public class RequestStepResult
    {
        ...
        internal void SetError(RequestError error)
        {
            if(this.Errors == null)
            {
                this.Errors = new List<RequestError>();
            }
            
            this.Errors.Add(error);
        }

        internal void SetErrors(List<RequestError> errors)
        {
            this.Errors = errors;
        }
        ...
        internal void SetErrorMessage(string errorMessage)
            => this.SetError(new RequestError(errorMessage));
